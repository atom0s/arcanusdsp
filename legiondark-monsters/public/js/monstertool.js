 ...
 * are added to the LegionDark website via the legiondark-monsters plugin.
 *
 * @author atom0s <atom0s@live.com>
 * @copyright Copyright (C) 2015-2016 atom0s [atom0s@live.com]
 */
legion.controller('monsterToolController', ['$filter', '$http', '$sce', '$scope', '$window', function ($filter, $http, $sce, $scope, $window) {
 ...
 */
legion.filter('respawnTime', function () {
    return function (val) {
        if (val == null || val === 0)
            return '00:00:00';
 ...
 */
legion.filter('spawnFlags', function () {
    return function (val) {
        if (val == null || val === 0)
            return 'Normal';
