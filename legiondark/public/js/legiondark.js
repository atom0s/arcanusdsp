...
// Initialize the LegionDark angular module..
var legion = angular.module('legion', []);

// Configure the module..
legion.config(['$httpProvider', function ($httpProvider) {
    // Make angular http requests seen as xhr by express.js..
    $httpProvider.defaults.headers.common['X-Requested-With'] = 'XMLHttpRequest';
}]);

/**
 * LegionDark Controller
 *
 * Controller that holds the main functionality of the LegionDark website.
 */
legion.controller('legionController', ['$filter', '$http', '$sce', '$scope', '$window', function ($filter, $http, $sce, $scope, $window) {
 ...
 */
legion.directive('repeatDone', function () {
 ...
 */
legion.filter('html', ['$sce', function ($sce) {
    return function (val) {
 ...
 */
legion.filter('cleanName', function () {
    return function (val) {
        if (!val)
            return 'Unknown';
 ...
 * @returns {string}                            The cleaned name.
 */
legion.filter('cleanNameTitleCase', function () {
    return function (val) {
        if (!val)
            return 'Unknown';
        val = val.replace(/_/g, ' ');
 ...
 */
legion.filter('jobInformation', function () {
    return function (p) {
 ...
 */
legion.filter('linkshellRankToImage', function () {
    return function (val) {
        switch (val) {
            case 1:
 ...
 */
legion.filter('nameflagsToImages', ['$sce', function ($sce) {
    return function (p) {
        var imgs = '';
 ...
 */
legion.filter('nationFlag', function () {
    return function (val) {
 ...
 */
legion.filter('nationName', function () {
    return function (val) {
 ...
 */
legion.filter('raceHairImage', function () {
    return function (p) {
        if (!p || p.race === 0)
 ...
 */
legion.filter('titleName', function () {
    return function (val) {
        var titles =
            [
                { id: 0, name: "N/A" },
 ...
 */
legion.filter('accountCharacterJobInformation', function () {
    return function (c) {
        if (!c)
            return '?/?';
 ...
 */
legion.filter('accountPrivileges', function () {
    return function (val) {
 ...
 * @returns {string}                The converted value.
 */
legion.filter('accountStatus', function () {
    return function (val) {
        var status = '(Unknown)';
